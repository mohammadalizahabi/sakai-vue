...
const { changeThemeSettings, setScale, layoutConfig, changeAlign, changeNoAlign } = useLayout();
...
const Align = () => { changeAlign(); }
const noAlign = () => { changeNoAlign(); }
...
<template>
    ...
    <Sidebar v-model:visible="visible" position="right" :transitionOptions="'.3s cubic-bezier(0, 0, 0.2, 1)'" class="layout-config-sidebar w-20rem">
        ...
        <h5>Right Align</h5>
            <InputSwitch v-if="!layoutConfig.ralign.value" v-model="layoutConfig.ralign.value" @click="Align()"></InputSwitch>
            <InputSwitch v-else-if="layoutConfig.ralign.value" v-model="layoutConfig.ralign.value" @click="noAlign()"></InputSwitch>
            <h5>Ripple Effect</h5>
